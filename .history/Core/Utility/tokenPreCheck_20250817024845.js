...
export async function _tokenPreCheck(db_handle, token, product) {
  try {
    ...
    if (changeEncountered) {
      ...
      const _updateToken = "UPDATE product SET ? = ? WHERE PRODUCT_ID = ?"
      ...
      if (!changeKey) {
        throw new Error("Unable to reference Key for updating product token");
      }
      await db_handle(_updateToken, [changeKey, mutableToken, productID])
