...
exports._tokenPreCheck = async (db_handle, token, product) => {
  try {
    // If the incoming token is already falsy, just return ""
    if (!token) return "";
    ...
    const _tokens = tokenParser(token); // linked list-like API
    ...
    // ✅ Always return a string, even if everything was removed
    return mutableToken || "";
  } catch (err) {
