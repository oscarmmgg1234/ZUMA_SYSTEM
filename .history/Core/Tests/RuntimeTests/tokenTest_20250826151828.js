...
<head>
  <meta charset="utf-8" />
  <title>Runtime Token Report</title>
...
const main = async (_params) => {
  ...
  try {
    ...
    const processesWithNames = await enrichValidArrWithNames(db_handle, _initProcess);
    const report = htmlReport(_initProcess);
