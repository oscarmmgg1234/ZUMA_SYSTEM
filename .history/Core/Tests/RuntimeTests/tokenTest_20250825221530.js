...
const initProcessFlows = async (db_handle, _default) => {
  try {
    ...
    if (_default.get("act").process_token) {
      ...
      _activationProcessValidation = activationResult.processValidation;
    }
    if (_default.get("red").process_token) {
      const reductionResult = await controller.reduction.product_reduction({
        ..._default.get("red"),
        transactionHandle: db_handle,
      });
      _reductionProcessValidation = reductionResult.result.processValidation;
    }
    ...
    if (_default.get("ship").process_token) {
      ...
      _shipmentProcessValidation =
        shipmentResult.execResponse[0].processValidation;
    }
    return {act: _activationProcessValidation, red: _reductionProcessValidation, ship: _shipmentProcessValidation}
