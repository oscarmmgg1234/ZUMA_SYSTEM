...
const getProductTokens = async (db_handle, productID) => {
  try {
    ...
    tokenMap.set("act", {token: product[0][0].ACTIVATION_TOKEN, product_name: product[0][0].NAME, productID: product[0][0].PRODUCT_ID});
    tokenMap.set("red", {
    ...
    tokenMap.set("ship", {
      ...
      product_name: product[0][0].NAME,, productID: product[0][0].PRODUCT_ID
    });
    return tokenMap;
  } catch (err) {
