...
const initProcessFlows = async (db_handle, _default) => {
  try {
    // helper: token exists and is not empty
    const hasToken = (key) => {
      const t = _default.get(key)?.process_token;
    ...
    let didActivation = false;
    let didShipment = false;
    ...
    // REDUCE (only if activation was executed)
    if (hasToken("red") && didActivation) {
    ...
    // SHIP
    if (hasToken("ship")) {
      console.log("ship payload:", _default.get("ship"));
    ...
    // SHIP REDUCTION (only if shipment executed)
    if (hasToken("shipRed") && didShipment) {
      console.log("shipRed payload:", _default.get("shipRed"));
...
<body>
  <div class="header">
    <h1>Runtime Token Report - ${mainProduct}</h1>
...
const main = async (_params) => {
  ...
  try {
    ...
    const mainProduct = await getProductName(db_handle, productID);
    ...
    // console.log(processesWithNames)
