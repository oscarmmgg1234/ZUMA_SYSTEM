...
const generateDefaultEngineInfo = async (db_handle, product) => {
  let outputMap = new Map();
  let actRoute = product.get("act")
  let shipRoute = product.get("ship")
  outputMap.set("act", {EMPLOYEE_ID: "000002", PRODUCT_ID: actRoute.productID, PRODUCT_NAME: actRoute.product_name})
  return outputMap;
};

const initProcessFlows = async (db_handle, product) => {
  try {
    let _reduction = [];
    let _activation = [];
    let _shipment = [];
...
const main = async (_params) => {
  ...
  try {
    ...
    const _initProcessInfo = await generateDefaultEngineInfo(
      db_handle,
      _productMap
    );
