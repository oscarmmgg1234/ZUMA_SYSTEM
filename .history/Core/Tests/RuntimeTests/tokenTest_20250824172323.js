...
const initProcessFlows = async (db_handle, _default) => {
  try {
    ...
    const activationPreProcess =
      await controller.product_activation_controller.activate_product(_default.get);
  } catch (err) {
    throw new Error(err);
  }
};
...
const main = async (_params) => {
  ...
  _barcodeID = generateRandomID(8);
  _redNewTransID = generateRandomID(8);
  try {
