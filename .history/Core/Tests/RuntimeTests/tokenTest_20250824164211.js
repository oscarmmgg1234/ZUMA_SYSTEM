...
const getProductTokens = async (db_handle, productID) => {
  try {
    ...
    tokenMap.set("act", {token: product[0][0].ACTIVATION_TOKEN, product_name: product[0][0].NAME});
    tokenMap.set("red", {token: product[0][0].REDUCTION_TOKEN);
    tokenMap.set("ship", {token: product[0][0].SHIPMENT_TOKEN);
    return tokenMap;
  } catch (err) {
