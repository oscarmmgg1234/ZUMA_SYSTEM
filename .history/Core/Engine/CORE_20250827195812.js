...
const core_exec = async (args, barcodeInput) => {
  try {
    ...
    if (!coreSuccess.success) {
      return {
        status: "error",
        message: "Core engine failed - Stock for a product might be 0 or below 0",
        product: {
    ...
    if (barcodeInput) {
      ...
      return {
        ...
        processValidation: coreSuccess?.processValidation,
      };
    }

    return {...success, processValidation: coreSuccess.processValidation};
  } catch (error) {
    return {
      status: "error",
      message: `Error: ${error.message} - Stock for a product might be 0 or below 0`,
      product: {
        id: args.PRODUCT_ID ? args.PRODUCT_ID : "Unknown",
        name: args.PRODUCT_NAME ? args.PRODUCT_NAME : "Unknown",
      },
