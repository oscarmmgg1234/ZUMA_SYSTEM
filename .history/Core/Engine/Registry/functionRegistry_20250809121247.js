    ...
    this.registry_map.set("4i57", {
      ...
      proto: async (db_handle, args, value, auxiliary) => {
        ...
        for(const item of linked_products){
          ...
          if(exist[0][0].length > 0){
            sanitizedLinkedProducts.push(item)
          }
        }
        //sanitize and then update the linked product to remove any products frontend in aciddently somehow pushed undefined pro
        await db_handle("UPDATE inv_virtual_stock SET LINKED_PRODUCTS = ? WHERE poolID", poolID)
