  ...
  init() {
    ...
    this.registry_map.set("4i57", {
      ...
      proto: async (db_handle, args, value, auxiliary) => {
        ...
        if (firstStageStatus) {
          const pool = await db_handle.raw("SELECT * FROM inv_virtual_stock WHERE poolID = ?", [auxiliary.auxiliaryParam])
          const product = JSON.parse(pool[0][0].LINKED_PRODUCTS).filter(item=>item.productID == value)
          
          //if error then it will return false then it will not run
          if (auxiliary.auxiliaryParam == "default") {
            ...
            //if its using ratio which whould be ratio * quantity
            args.recordHandler.step({
              normalize: true,
              column: "STORED_STOCK",
              value:
