  ...
  init() {
    ...
    this.registry_map.set("4i57", {
      ...
      proto: async (db_handle, args, value, auxiliary) => {
        ...
        //So think about how we are goign to implement this, wether user defines the ratio at link creation
        // Or let user define tokens to specify this.
        // we filter out the current product token

        //class:funcid:productID:poolID:ratio
        ...
        const ratio = auxiliary.nextAuxiliaryParam;
        ...
        v
         ...
         const pool = await db_handle.raw(
          ...
          const mainProductProcess = linked_products.filter(item=>item.productID ==mainProductID);


         if(mainProductProcess.lenght < 1){
          return;
         }
        ...
        await db_handle.raw(
          ...
          const shared_stock = poolData[0][0].STOCK;
        ...
        await db_handle("UPDATE inv_virtual_stock SET LINKED_PRODUCTS = ? WHERE poolID = ?", [JSON.stringify(sanitizedLinkedProducts), poolID])
