...
class FunctionRegistry {
  ...
  /**
   * Build a unified params[] spec from legacy counts/descs when no explicit override exists.
   * Keeps productId hidden and ordered first, then required "mains" (after productId), then optionals.
   */
  buildParamsFromLegacy(meta = {}) {
    const rawMain = Math.max(0, Number(meta.mainParams || 0)); // includes productId
    const visibleMain = Math.max(0, rawMain - 1); // user-entered mains (after productId)
    const optCount = Math.max(0, Number(meta.optionalParams || 0));
    const descs = Array.isArray(meta.optionalDesc) ? meta.optionalDesc : [];
    const includesProductIdDesc = descs.length === rawMain + optCount; // descs includes productId row

    const getDesc = (idx, fallback) =>
      (descs[idx] && (descs[idx].desc || descs[idx])) || fallback;

    const params = [
      { key: "productId", role: "productId", hidden: true, required: true },
    ];

    // visible "mains" (required) after productId
    for (let i = 0; i < visibleMain; i++) {
      const base = includesProductIdDesc ? 1 : 0; // skip productId label if present
      params.push({
        key: `main_${i}`,
  ...
  /**
   * Apply explicit param overrides for hot functions, then backfill the rest
   * from legacy counts via buildParamsFromLegacy. Idempotent and safe to re-run.
   */
  applyParamSpecs() {
    const overrides = {
      // ======= AC =======
      1023: {
        params: [
          ...
          {
            key: "customAmount",
            label: "Custom amount (overrides quantity)",
            type: "number",
            required: false,
            hint: "Multiplied by MULTIPLIER if present",
          },
        ],
      },
      "290W": {
      ...
      "739e": {
        params: [
          ...
          {
            key: "customPartial",
            label: "Partial multiplier × quantity",
            type: "number",
            required: false,
          },
        ],
      ...
      "2j3w": {
        params: [
          ...
          {
            key: "customAmount",
            label: "Custom amount (overrides activated qty)",
            type: "number",
            required: false,
          },
        ],
      ...
      "2a1k": {
        // NOTE: your impl expects a ratio; keep optional to preserve legacy behavior
        params: [
          { key: "productId", role: "productId", hidden: true, required: true },
          {
            key: "pillRatio",
      ...
      "2q3e": {
        params: [
          ...
          {
            key: "ratioA",
            label: "Set glycerin ratio for product",
            type: "number",
            required: false,
          },
          {
          ...
          {
            key: "oz",
            label: "Set glycerin ratio in oz (typically 26)",
            type: "number",
            required: false,
          },
        ],
    ...
    for (const [id, fn] of this.registry_map.entries()) {
      const md = fn.meta_data || {};
      // If override exists, use it; otherwise backfill from legacy only if params[] missing
      const override = overrides[id];
      const params =
        override?.params || md.params || this.buildParamsFromLegacy(md);
      ...
      this.registry_map.set(id, {
        ...
        meta_data: {
          ...md,
          params,
          paramSchemaVersion: 1, // marker so the UI can branch cleanly
        },
      });
    }
  }
  ...
  applyEffectTags() {
    const EFFECTS = {
      // ---------- AC ----------
      1023: ["receipt"], // Insert Activation Record (record only)
      "290W": ["receipt", "active up"],

      // ---------- RD ----------
      "29ew": ["receipt", "stored down"],
      "29wp": ["receipt", "stored down"],
      "10fd": ["receipt"],
      "739e": ["receipt"],
      "2js2": ["receipt"],
      "234d": ["receipt"],
      ...
      // ---------- CM / CMUP ----------
      "50wk": ["receipt"],
      "34fk": ["receipt", "active down"],
      "2j2k": ["receipt", "active down"],
      "10fj": ["stored down"],
      "13g4": ["active down"],

      // ---------- SH ----------
      "38dh": ["receipt", "shipment"],
      "23ij": ["receipt", "stored up"],
    };
    ...
    for (const [id, effs] of Object.entries(EFFECTS)) {
      ...
      this.registry_map.set(id, {
        ...fn,
        meta_data: { ...(fn.meta_data || {}), effect: effs },
      });
    }
  }
  ...
  applyFriendlyLabels() {
    const updates = {
      ...
      "2j2k": {
        ...
        name: "New v2 • Reduce Stored with Receipt",
        desc:
