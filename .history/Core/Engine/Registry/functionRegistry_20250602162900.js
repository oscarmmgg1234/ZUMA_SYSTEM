...
class FunctionRegistry {
  ...
  init() {
    ...
    this.registry_map.set("290W", {name: "Activate Product on 1 on 1 basis", desc: "Activate product -> option to completely activate without aditional steps", meta_data: {}, class: "AC", proto: async (db_handle, args, value, auxiliary) => {
      // activate so combine the porducts
      const AC_FUNCTION = this.getFunction("1023");
      const ACUP_FUNCTION = this.getFunction("2j3w");
      //insert activation record
      await AC_FUNCTION.proto(db_handle, args, value, auxiliary);
      //update product stored stock subtract
      await ACUP_FUNCTION.proto(db_handle, args, value, auxiliary);


    }});
    this.registry_map.set("29ew", {name: "Reduce Product in 1 on 1 basis", desc: "Reduce product -> option to completely reduce without aditional steps", meta_data: {}, class: "RD", proto: async (db_handle, args, value, auxiliary) => {
