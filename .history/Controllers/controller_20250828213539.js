...
//==================================================================================================
//Companies Setters
const changeCompanyInfo = async (args) => {
  const { changes, companyID } = args;
  //changes [{key: NAME, value: "ecma" },...]
  try {
    const db_handle = await transactionUnit();
    for (const item of changes) {
      await db_handle.raw("UPDATE company SET ?? = ? WHERE COMPANY_ID = ?", [item.key, item.value, companyID]);
    }
  } catch (err) {
...
const getCompaniesWithProducts = async () => {
  try {
    const db_handle = await transactionUnit();
    //get companies
    const getComp = await db_handle.raw("SELECT * FROM company");
    const companies = getComp[0];
    let outputArr = [];
    for (const company of companies) {
      ...
      if (products[0].length < 1) {
        outputArr.push({ company: company, productArr: null });
        continue;
      }
      outputArr.push({ company: company, productArr: products[0] });
    }
    return outputArr;
  } catch (err) {
    throw new Error("Could not retrieve companies with products");
  }
};

const updateProductCompany = async (args) => {
  const { productID, companyID, option } = args;
  try {
    const query = "UPDATE product set COMPANY = ? WHERE PRODUCT_ID = ?";
    const db_handle = await transactionUnit();
    if (option == "single") {
      const updateProduct = await db_handle.raw(query, [companyID, productID]);
      return { success: true };
    } else {
      const productsWithCompanies = await getCompaniesWithProducts();
      const companyWithProducts = productsWithCompanies.filter(
        (item) => item.company.COMPANY_ID === companyID
      );
      for (const item of companyWithProducts[0].productArr) {
        const updateAllProducts = await db_handle.raw(query, [
          companyID,
          item.PRODUCT_ID,
        ]);
      }
...
const manageCompanies = async (args) => {
  const { NAME, ADDRESS, TYPE, PHONE, COMPANY_ID, option } = args;
  try {
    const db_handle = await transactionUnit();
    if (option == "create") {
      const insertNewCompany = await db_handle.raw(
