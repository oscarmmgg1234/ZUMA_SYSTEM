...
const delete_product = async (args) => {
  try {
    ...
    throw new Error("could not resolve deletion of product");
  }
};
...
const deleteProduct = async (args) => {
  ...
  try {
    ...
    if (product[0][0].poolRef) {
      const response = await updateVirtualPoolRefs({
        process: "unlink",
    ...
    console.log("2")
    const deletion = await delete_product(args);
    return { success: true, err: err, deletion: deletion };
  } catch (err) {
