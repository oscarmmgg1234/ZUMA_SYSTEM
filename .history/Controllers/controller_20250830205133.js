...
//==================================================================================================
//TYPE Setters
const changeTypeInfo = async ({ changes, typeID }) => {
  if (!changes || changes.length < 1) return { success: true };

  const db_handle = await transactionUnit();
  try {
    for (const { key, value } of changes) {
      // keys should be one of: TYPE, DESCRIPTION, RISK
      await db_handle.raw("UPDATE product_type SET ?? = ? WHERE TYPE_ID = ?", [
        key,
        value,
        typeID,
      ]);
...
// Get all types with their products (product.TYPE matches product_type.TYPE_ID)
const getTypesWithProducts = async () => {
  const db_handle = await transactionUnit();
  try {
    const [types] = await db_handle.raw("SELECT * FROM product_type");
    const outputArr = [];

    for (const typeRow of types) {
      const [products] = await db_handle.raw(
        "SELECT * FROM product WHERE TYPE = ?",
        [typeRow.TYPE_ID]
      );
      outputArr.push({
        type: typeRow,
        productArr: products.length ? products : null,
      });
    }
    ...
    await db_handle.rollback();
    throw new Error("Could not retrieve types with products: " + err.message);
  }
};

// Update product.TYPE to a given TYPE_ID (single product or all products that already belong to that TYPE_ID)
const updateProductType = async ({ productID, typeID, option }) => {
  const db_handle = await transactionUnit();
  const query = "UPDATE product SET TYPE = ? WHERE PRODUCT_ID = ?";

  try {
    if (option === "single") {
      await db_handle.raw(query, [typeID, productID]);
    } else {
      // Bulk: iterate products currently under this typeID and (re)set their TYPE to typeID.
      // (If you want to *move* between types, see bulkMoveProductsBetweenTypes below.)
      const typesWithProducts = await getTypesWithProducts();
      const typeWithProducts = typesWithProducts.find(
        (item) => item.type.TYPE_ID === typeID
      );

      if (typeWithProducts?.productArr) {
        for (const product of typeWithProducts.productArr) {
          await db_handle.raw(query, [typeID, product.PRODUCT_ID]);
        }
      }
    }
    await db_handle.commit();
...
// Create/delete rows in product_type. TYPE_ID must be provided on create; we do not edit TYPE_ID here.
const manageTypes = async ({ TYPE_ID, TYPE, DESCRIPTION, RISK, option }) => {
  const db_handle = await transactionUnit();
  try {
    if (option === "create") {
      await db_handle.raw(
        "INSERT INTO product_type(TYPE_ID, TYPE, DESCRIPTION, RISK) VALUES (?,?,?,?)",
        [TYPE_ID, TYPE, DESCRIPTION, RISK]
      );
    } else if (option === "delete") {
      await db_handle.raw("DELETE FROM product_type WHERE TYPE_ID = ?", [
        TYPE_ID,
      ]);
    } else {
      throw new Error("Invalid option for manageTypes");
    }
...
//==================================================================================================
//Companies Setters
const changeCompanyInfo = async ({ changes, companyID }) => {
  ...
  try {
    for (const { key, value } of changes) {
      await db_handle.raw("UPDATE company SET ?? = ? WHERE COMPANY_ID = ?", [
        key,
        value,
        companyID,
      ]);
...
const getCompaniesWithProducts = async () => {
  const db_handle = await transactionUnit();
  try {
    const [companies] = await db_handle.raw("SELECT * FROM company");
    const outputArr = [];
    ...
    await db_handle.rollback();
    throw new Error(
      "Could not retrieve companies with products: " + err.message
    );
  }
};

const updateProductCompany = async ({ productID, companyID, option }) => {
  const db_handle = await transactionUnit();
  const query = "UPDATE product SET COMPANY = ? WHERE PRODUCT_ID = ?";

  try {
    if (option === "single") {
      await db_handle.raw(query, [companyID, productID]);
    } else {
      const companiesWithProducts = await getCompaniesWithProducts();
      const companyWithProducts = companiesWithProducts.find(
        (item) => item.company.COMPANY_ID === companyID
      );

      if (companyWithProducts?.productArr) {
        for (const product of companyWithProducts.productArr) {
          await db_handle.raw(query, [companyID, product.PRODUCT_ID]);
        }
      }
    }
    await db_handle.commit();
  ...
  try {
    if (option === "create") {
      await db_handle.raw(
        ...
        [COMPANY_ID, NAME, ADDRESS, TYPE, PHONE]
      );
    } else if (option === "delete") {
      await db_handle.raw("DELETE FROM company WHERE COMPANY_ID = ?", [
        COMPANY_ID,
      ]);
    } else {
      throw new Error("Invalid option for manageCompanies");
    }
...
const tokenPreCheck = async ({ token, productID, route, mode = "commit" }) => {
  ...
  try {
    ...
    try {
      if (db_handle) await db_handle.rollback();
    } catch (_) {}
    throw err;
  }
};
...
const delete_product = async (args) => {
  const status = await knex.raw("DELETE from product WHERE PRODUCT_ID = ?", [
    args.productID,
  ]);
  return status;
};
...
const deleteProduct = async (args) => {
  ...
  try {
    ...
    if (!product) {
      return { success: false, err: "PRODUCT_NOT_FOUND", deletion: null };
    }

    // Unlink if needed (ensure this returns a Promise!)
    if (product.poolRef) {
      await updateVirtualPoolRefs({
        process: "unlink",
        poolID: product.poolRef,
        productID: args.productID,
        normalizeRatio: 1,
      });
    }

    const deletion = await delete_product(args, knex);
    return { success: true, err: null, deletion };
  } catch (err) {
    console.error("deleteProduct service error:", err);
...
const activate_product = async (args) => {
  ...
  let db_handle = args.transactionHandle ? args.transactionHandle : null;

  db_handle
    ? await db_handle.raw(addTrans, [
        args.TRANSACTIONID,
        JSON.stringify([]),
        JSON.stringify([]),
        JSON.stringify([]),
        ...
        args.PRODUCT_ID,
        args.MULTIPLIER
          ? parseInt(args.MULTIPLIER) * args.QUANTITY
          : args.QUANTITY,
      ])
    : await knex.raw(addTrans, [
        args.TRANSACTIONID,
        JSON.stringify([]),
        JSON.stringify([]),
        JSON.stringify([]),
  ...
  const barcodeID = args.generatedBarcodeID
    ? args.generatedBarcodeID
  ...
  const barcodeInput = {
    ...
    id: barcodeID,
    TRANSACTIONID: args.TRANSACTIONID,
    transactionHandle: db_handle ? db_handle : null,
  };
  if (db_handle) {
    //  db(
    ...
    //             args.TRANSACTIONID,
    //           ],
    await db_handle.raw(queries.tools.barcode_log, [
      barcodeID,
      args.EMPLOYEE_ID,
      args.PRODUCT_NAME,
      args.QUANTITY,
...
const product_reduction = async (args) => {
  try {
    let db_handle = args.transactionHandle ? args.transactionHandle : null;
    const validator = db_handle
      ? await db_handle.raw("SELECT * FROM barcode_log WHERE BarcodeID = ?", [
          args.BARCODE_ID,
        ])
      : await knex.raw("SELECT * FROM barcode_log WHERE BarcodeID = ?", [
          args.BARCODE_ID,
        ]);
    ...
    if (
      ...
      return {
        ...
        params: { _barcode: args.BARCODE_ID, _trans: args.TRANSACTIONID },
        result,
      };
    } else {
      return {
        status: false,
        message: "Product Already Reduced",
        sucess: false,
      };
    }
  } catch (error) {
    return { status: false, message: error, sucess: false };
  }
};
...
const shipment_add = async (args) => {
  ...
  try {
    ...
    let execResponse = [];
    // await db_api.addTransaction({ src: "activation", args: args });
    let db_handle = args[0]?.transactionHandle
      ? args[0]?.transactionHandle
      : null;
    // await db_handle.raw(addTrans, [
    //   args.TRANSACTIONID,
    //   JSON.stringify([]),
    ...
    if (db_handle) {
      //barcode with generated shipment barcode
      await db_handle.raw(queries.tools.barcode_log, [
        args[0].generatedBarcodeID,
        args[0].EMPLOYEE_ID,
        args[0].PRODUCT_NAME,
        args[0].QUANTITY,
        "Active/Passive",
    ...
    for (const shipmentObject of args) {
      db_handle
        ? await db_handle.raw(addTrans, [
            ...
            0,
            "shipment",
            shipmentObject.EMPLOYEE_ID,
            shipmentObject.PRODUCT_ID,
            shipmentObject.MULTIPLIER
              ? parseInt(shipmentObject.MULTIPLIER) * shipmentObject.QUANTITY
...
class controller {
  ...
  dashboard_controller = {
    // --- TYPE CONTROLS ---
    updateTypeInfo: async (args) => {
      return await changeTypeInfo(args);
    },
    manageTypes: async (args) => {
      return await manageTypes(args);
    },
    updateProductType: async (args) => {
      return await updateProductType(args);
    },
    getTypesWithProducts: async () => {
      return await getTypesWithProducts();
    },
