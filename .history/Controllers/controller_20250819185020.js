...
const deleteProduct = async (args, callback) => {
  //if product linked then we must unlink it
  const product = await knex.raw("SELECT poolRef from product WHERE PRODUCT_ID = ?"); //either be null or id
  if(product[0][0].poolRef){
    await updateVirtualPoolRefs({process: "unlink", poolID: product[0][0].poolRef, productID: args.PRODUCT_ID})
  }
