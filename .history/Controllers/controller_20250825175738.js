...
const activate_product = async (args) => {
  ...
  let db_handle = args.transactionHandle ? args.transactionHandle : ;
  await db_handle.raw(addTrans, [
    ...
    args.EMPLOYEE_ID,
    args.PRODUCT_ID,
    args.MULTIPLIER ? parseInt(args.MULTIPLIER) * args.QUANTITY : args.QUANTITY,
  ]);
...
const product_reduction = async (args) => {
  try {
    ...
    const validator = await db_handle.raw(
    ...
    if (
      ...
      const retriveToken = await db_handle.raw(
